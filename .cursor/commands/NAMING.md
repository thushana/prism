...
- **No abbreviations:** Avoid acronyms and shortened forms unless in this allowlist: `id`, `ids`, `url`, `urls`, `html`, `css`, `api`, `ui`, `ux`, and framework-defined names such as `config` in `next.config.js`. Outside these cases, prefer full words: `background` not `bg`, `database` not `db`, `configuration` not `config` in your own code.
