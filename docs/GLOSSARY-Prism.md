...
**Context**: Run from the child app root via `pnpm run prism:sync` (full pipeline) or the narrower `prism:sync:*` scripts. See [SYNC-Prism.md](./SYNC-Prism.md) for intent, commands, and submodule commit order.
