...
export interface FeatureFlagContext {
  ...
  /** URL overrides from ?param=value (set by middleware from query string) */
...
export interface CreateIdentifyOptions {
  ...
  authCheck?: (cookies: { get: (name: string) => { value: string } | undefined }) => Promise<FeatureFlagContext["user"]>;
...
export interface CreateFlagConfig<T> {
  ...
  origin?: string;
  identify: (args: { headers: Headers; cookies: { get: (name: string) => { value: string } | undefined } }) => Promise<FeatureFlagContext> | FeatureFlagContext;
  decide: (context: FeatureFlagContext) => T;
}

/** Config for getMiddleware() */
