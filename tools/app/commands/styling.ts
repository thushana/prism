...
import {
  ...
  colorNames,
  materialColors,
} from "../../../packages/cli/source/styling";
...
export async function runStylingCommand(
  ...
  logger.info(
    chalk.bold.cyan("\n📋 ") +
      chalk.bold("💎 Prism") +
      chalk.bold.cyan(" CLI Style Sheet\n")
  );
  ...
  for (const colorName of colorNames) {
    ...
    logger.info(
      `    ${lightSample}  ${color.light.bgBlack.bold("light")}    ${color.light.bgBlack("50".padStart(3))}  ${color.light.bgBlack(hex[50])}`
    );
    logger.info(
      `    ${defaultSample}  ${color.default.bold("default")}  ${color.default("600".padStart(3))}  ${color.default(hex[600])}`
    );
    logger.info(
      `    ${darkSample}  ${color.dark.bold("dark")}     ${color.dark("900".padStart(3))}  ${color.dark(hex[900])}`
    );
    logger.info("");
  }
  ...
  logger.info(
    `  ${styles.success("success")}  ${chalk.dim("Green #43A047")} - Success messages`
  );
  logger.info(
    `  ${styles.error("error")}  ${chalk.dim("Red #E53935")} - Error messages`
  );
  logger.info(
    `  ${styles.warning("warning")}  ${chalk.dim("Amber #FFB300")} - Warning messages`
  );
  logger.info(
    `  ${styles.info("info")}  ${chalk.dim("Blue #1E88E5")} - Info messages`
  );
  ...
  logger.info(
    `  ${chalk.cyan.underline("chalk.cyan.underline()")} - Cyan underlined`
  );
  logger.info(`  ${chalk.magenta.dim("chalk.magenta.dim()")} - Magenta dimmed`);
  logger.info(
    `  ${chalk.yellow.bgBlack("chalk.yellow.bgBlack()")} - Yellow on black`
  );
  ...
  const bar25 =
    colors.blue.default("█".repeat(filled25)) +
    chalk.dim("░".repeat(barLength - filled25));
  logger.info(`  ${bar25} 25/100 (25.0%)`);

  // Example 2: 50% progress
  const filled50 = Math.round((50 / 100) * barLength);
  const bar50 =
    colors.cyan.default("█".repeat(filled50)) +
    chalk.dim("░".repeat(barLength - filled50));
  logger.info(`  ${bar50} 50/100 (50.0%)`);

  // Example 3: 75% progress
  const filled75 = Math.round((75 / 100) * barLength);
  const bar75 =
    colors.amber.default("█".repeat(filled75)) +
    chalk.dim("░".repeat(barLength - filled75));
  ...
  logger.info(
    chalk.dim("  ? Select items: (Press <space> to select, <a> to toggle all)")
  );
